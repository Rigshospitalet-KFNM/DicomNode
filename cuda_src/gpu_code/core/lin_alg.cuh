...
namespace LINALG {
  namespace DEVICE {
    ...
    __device__ void swapRow(volatile SquareMatrix<DIMENSION>* matrix_,
                            const u8 r1, const u8 r2){
      volatile SquareMatrix<DIMENSION>& matrix = *matrix_;
      const u32 matrixRow = threadIdx.x / DIMENSION;
      const u32 matrixCol = threadIdx.x % DIMENSION;
...
#pragma unroll
    ...
    __device__ dicomNodeError_t _invertMatrixForward(
      ...
      const u32 matrixCol = threadIdx.x % DIMENSION;
      const u32 matrixRow = (threadIdx.x / DIMENSION) % DIMENSION;
      const bool outputThread = active && DIMENSION * DIMENSION <= threadIdx.x;
      const u32 output_thread_idx = threadIdx.x % (DIMENSION * DIMENSION);
