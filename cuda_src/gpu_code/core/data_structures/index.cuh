...
struct Index {
  ...
  constexpr __device__ __host__ Index(const T& arr, cuda::std::index_sequence<idx_seq...>)
    noexcept : coordinates{static_cast<i32>(arr[idx_seq])...} { }

  template<typename T>
  constexpr __device__ __host__ Index(const T (&arr)[DIMENSIONS]) noexcept :
    Index(arr, cuda::std::make_index_sequence<DIMENSIONS>()) {}

  template<typename... Args>
  constexpr __device__ __host__ Index(const Args... args) noexcept
    : coordinates{static_cast<i32>(args)...} {
    static_assert(sizeof...(args) == DIMENSIONS);
  };

  constexpr __device__ __host__ Index(const u64& flat_index, const Extent<DIMENSIONS>& space){
  ...
  constexpr __device__ __host__  i32& operator[](const u8 idx){
    // you can't put a static assert in here :(
    return coordinates[idx];
  }

  constexpr __device__ __host__ i32 operator[](const u8 idx) const {
    return coordinates[idx];
  }

  constexpr __device__ __host__ Index<DIMENSIONS> operator+(const Index<DIMENSIONS>& other) const {
  ...
  constexpr __device__ __host__ const i32& x() const {
    return coordinates[0];
  }

  constexpr __device__ __host__ const i32& y() const {
    static_assert(DIMENSIONS > 1);
    return coordinates[1];
  }

  constexpr __device__ __host__ const i32& z() const {
