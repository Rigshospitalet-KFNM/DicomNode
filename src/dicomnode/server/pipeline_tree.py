...
class PatientNode(ImageTreeInterface):
  ...
  def validate_inputs(self) -> bool:
    """Checks if the node is ready for processing or is missing images

    Raises:
        InvalidTreeNode: If the pipeline tree structure have been violated!

    Returns:
        (bool): if PatientNode ready (True) or not(False)
    """
    valid = True
    for dicomnode_input in self.data.values():
      if isinstance(dicomnode_input, AbstractInput):
